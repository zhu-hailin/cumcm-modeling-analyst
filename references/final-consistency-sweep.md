...
先判断修改真正影响了什么，再决定重跑范围；问题级别高不等于必须重算所有模型：

| 修改 | 必要动作 | 不应自动做 |
|---|---|---|
| 仅错别字、注释、引用或排版 | 核对未改变含义，重导出并检查受影响文档 | 重训模型或重审题包 |
| 仅图中文字、字体、颜色、尺寸 | 从同一正式数据重绘，核对标签/数值与最终尺寸 | 重跑数值求解 |
| 统计汇总、指标计算或舍入口径 | 重算相应统计并更新受影响表图/正文 | 重做不相关模型 |
| 数据、关键假设、约束或计算逻辑 | 新运行和验证，沿真实依赖更新下游 | 未确认的后问自动求解 |

需要计算重跑的修改，沿以下链检查；无关分支不重复验收：
